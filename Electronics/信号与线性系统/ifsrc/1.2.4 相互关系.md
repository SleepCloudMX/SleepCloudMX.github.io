...
\begin{align}
\delta^{(n)}(t)
&\rla \inti \delta^{(n)}(t) \e^{-\j\omega t} \dt
= (-1)^n \eval{
	\dv[n]{t} \e^{-\j\omega t}
}_{t = 0}
= (\j\omega)^n,
...
$$
t^k \delta^{(n)}(t) \rla \dfrac{n!}{(n-k)!} \j^{n + k} \omega^{n-k},
$$
将 $n$ 替换为 $ n + k $, 有
$$
t^k \delta^{(n + k)}(t) \rla
\dfrac{(n + k)!}{n!} (-1)^k (\j\omega)^n,
$$
记 $ m = n + k $, 则
$$
\dfrac{n!}{m!} (-t)^{m-n} \delta^{(m)}(t)
\rla (\j\omega)^n,
$$
而之前我们已经得到了 $ \delta^{(n)}(t) \rla (\j\omega)^n $, 由傅里叶逆变换的唯一性, 有
$$
\dfrac{(-t)^m}{m!} \delta^{(m)}(t)
= \dfrac{(-t)^n}{n!} \delta^{(n)}(t),
$$
代入 $ m = 0 $, 即有
$$
...
$$
\begin{align}
t^n \delta^{(n)}(t) &= \dsum_{i=0}^n (-1)^i \binom{n}{i} f^{(i)}(0) \delta^{(n-i)}(t) \\
&= (-1)^n n! \delta(t),
\end{align}
$$
...
2. 假设对于某个正整数 $n$, 有 $ \delta^{(n)}(t) = \dfrac{n!}{(-t)^n} \delta(t) $, 则
   $$
   \begin{align}
   \delta^{(n+1)}(t) &= \dfrac{n!}{(-t)^n} \delta'(t) + \dfrac{n \cdot n!}{(-t)^{n+1}} \delta(t)
   \\
   &= \dfrac{n!}{(-t)^{n+1}} \delta(t) + \dfrac{n \cdot n!}{(-t)^{n+1}} \delta(t)
   \\
   &= \dfrac{(n+1)!}{(-t)^{n+1}} \delta(t),
   \end{align}
   $$
   即对于 $ n + 1 $ 也成立. 证毕.
...
$$
\inti \dfrac{(-t)^n}{n!} \delta^{(n)}(t) \dt = 1,
$$
此外, 当 $ t \ne 0 $ 时, $ \dfrac{(-t)^n}{n!} \delta^{(n)}(t) = 0 $, 于是满足冲激函数的定义, 即
$$
\delta(t) = \dfrac{(-t)^n}{n!} \delta^{(n)}(t).
$$
证毕.