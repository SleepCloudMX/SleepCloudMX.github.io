...
\begin{align}
f'(t) &\lt \intoi f'(t) \e^{-st} \dt
\\
&= \eval{f(t) \e^{-st}}_{0_-}^{+\infty} +
\intoi f(t) \e^{-st} \dt
\\
&= s F(s) - f(0_-).
\end{align}
...
$$
\begin{align}
F(s) &= \intoi f(t) \e^{-st} \dt
\\
F'(s) &= \intoi \bqty{-t f(t)} \e^{-st} \dt.
\end{align}
$$
...
注意这里 $ f(t) u(t) \lt F(s) $，上面的 $ F(\j\omega) $ 即把 $s$ 替换为 $j$. 从而有
$$
\cal L\bqty{f'(t)}(s)
...
由 $ f(t) u(t) \ft F(\j\omega) $，我们有
$$
-\j t f(t) u(t) \ft \j F'(\j\omega),
$$
于是 $ -f(t) u(t) \lt F'(s) $. 证毕.