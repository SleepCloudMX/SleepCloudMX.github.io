...
- 传输函数可以看作算子的单值函数, 也可以看作泛函, 即 $ H(p)(f(t)) $. 对于后者, <span style="color: red">传输函数不一定是单值的</span> (这也就是我在推导下述结论时发生矛盾的原因).
...
- $ \dfrac{A}{B_2} \cdot \dfrac{1}{B_1} f \subseteq \dfrac{A}{B_1 B_2} f $.
- $ \dfrac{A}{B} f \subseteq \dfrac{1}{C} \cdot \dfrac{AC}{B} f = \dfrac{AC}{BC} f $.
- $ C \cdot \dfrac{A}{BC} f = \dfrac{A}{B} f $.
...
压根没学过泛函分析, 不懂广义函数论, 草草学完了抽象代数也从不复习, 我的数学基础真是太差了 (哭).
