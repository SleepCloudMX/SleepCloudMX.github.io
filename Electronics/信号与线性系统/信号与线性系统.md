...
- 时域抽样定理
  ...
  - 采样的分类
    ...
    - 当 $ \omega_\text s > 2\omega_\text m $ 时, 称为 **过采样**, 可用低通滤波器恢复出原信号.
  - 其它概念
...
t^k \delta^{(n)}(t) \ft
(n)_{(k)} \j^n \omega^{n-k},
& t^n \ft \dfrac{2\pi \j^{n - k}}{(n+k)_{(k)}}
...
<div style="background-color: #f3f2ee">
    <details>
        ...
        <iframe src="ifsrc\A.8.1 冲激信号.html" height=550></iframe>
    </details>
</div>
