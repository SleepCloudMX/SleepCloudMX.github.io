...
- 零输入响应
  - $ r_{\text{zi}}(t) = H[r^{(k)}(0_-)] $. (齐次解的一部分)
  - 解齐次方程, 以 $ r^{(k)}(0_-) $ 为初值.
- 零状态响应
  - $ r_{\text{zs}}(t) = H[e(t)] $. (齐次解的一部分 + 特解)
  - 解非齐次方程, 以 $ r_\text{zs}^{(k)}(0_+) = r^{(k)}(0_+) - r^{(k)}(0_-) $ 为初值.
...
2. $ f^{*-1} = \cal F\inv [\cal F[\delta] \cdot \cal F[f]] $.
3. $ f*g=h \RA g = \cal F\inv[\cal F[h] / \cal F[f]] $.
...
这两种方法本质上是一样的, 只是前者侧重代数, 后者侧重几何. 个人认为前者更加简便.
