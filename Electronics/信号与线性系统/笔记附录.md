...
- [x] [第三章课后题](hw\教材课后题\第三章.html).
- [ ] [第四章课后题](hw\教材课后题\第四章.html).
- [ ] [第五章课后题](hw\教材课后题\第五章.html).
...
前三行只需证 $ \dfrac{1}{(z - a)^k} \zt a^{n-k} \dbinom{n-1}{k-1} u(n - 1) $，右侧式子是其特例，左侧式子对其使用位移性质即可. 由数学归纳法，

1. 当 $ k = 1 $ 时，幂级数展开左式即可，或者对右式使用 z 变换定义即得.
2. 假设取 $k$ 时上式成立，则

$$
\begin{align}
\dfrac{1}{(z - a)^{k + 1}}
&\zt a^{n-k} \binom{n-1}{k-1} u(n-1) * a^{n-1} u(n-1)
\\
&= \dsum_{m=1}^{n-1} a^{n-m-1} a^{m-k} \binom{m-1}{k-1} u(n-1)
\\
&= a^{n-k-1} u(n-1) \dsum_{m=1}^{n-1} \binom{m-1}{k-1}
\\
&= a^{n-k-1} \binom{n-1}{k} u(n-1).
\end{align}
$$
