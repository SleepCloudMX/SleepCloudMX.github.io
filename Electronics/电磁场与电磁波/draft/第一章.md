...
1. $ \curl (\curl \bm a) = \grad (\div \bm a) - \div (\grad \bm a) $.
2. $ (\curl \grad) \cp \bm a = \bm 0 $.
3. $ \bm a \cp (\curl \bm b) = (\grad \bm a) \bm\cdot \bm b - (\bm a \bm\cdot \grad) \bm b $.（误）
4. $ (\curl \bm b) \cp \bm a = (\bm a \bm\cdot \grad) \bm b - (\grad \bm a) \bm\cdot \bm b $.（误）
5. $ (\bm a \cp \grad) \cp \bm b $
...
3. $ \grad (\bm a \bm\cdot \bm b) = \bm a \bm\cdot \grad \bm b + \bm b \bm\cdot \grad \bm a $.（误）

4. $ \grad (\bm a \bm\cdot \bm b) = \bm a \times (\curl \bm b) + \bm b \times (\curl \bm a) + (\bm a \bm\cdot \grad) \bm b + (\bm b \bm\cdot \grad) \bm a $.

5. $ \grad (\div \bm a) = \Delta \bm a + \curl(\curl \bm a) $.

**证明**

这里下标为数字则表示分量，为字母则表示偏导.
$$
\begin{align}
\grad (\lambda f + \mu g)
&= (\lambda f_x + \mu g_x) + (\lambda f_y + \mu g_y) + (\lambda f_z + \mu g_z)
\\
...
$$
\begin{align}
\grad (fg) &= (f_x g + f g_x) + (f_y g + f g_y) + (f_z g + f g_z)
\\
...
% &= \sum a_{1x} b_1 + \sum a_1 b_{1x}
= \bm a \bm\cdot \grad \bm b + \bm b \bm\cdot \grad \bm a.
\end{align}
$$
