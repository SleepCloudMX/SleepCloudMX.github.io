...
若解析函数 $ z \mapsto \wt z = f(z) $ 是一个运动, 则
$$
...
:star: 若绕<u>单位向量</u> $ \bm v = l \bm i + m \bm j + n \bm k $ 旋转,端点为 $ \hat a $, 球极射影象为 $ a $, 则
$$
...
- 四元数 $ \mathbb V $ 
  ...
  - 备注
    ...
    - 这里四元数单位使用粗斜体, 表明它是 (四维) 向量或 (二阶) 矩阵.
    - 有的资料上使用正体表示四元数单位, 表明它是标量常数.
  - 性质
...
- 空间旋转
  ...
  - 旋转
    ...
    - **旋转的矩阵**
      ...
      $$
      \R_\bm v^\psi = \cos \dfrac{\psi}{2} + \bm V \sin \dfrac{\psi}{2}
      $$

    - $ \cal R_{\hat{\wt{p}}}^\theta = \cal R_\hat a^\psi \circ \cal R_\hat p^\theta \circ \cal R_\hat a^{-\psi} $, 即 $ \bb R_\wt{\bm P}^\theta = \bb R_\bm v^\psi \circ \bb R_\bm P^\theta \circ \bb R_\bm v^{-\psi} $.

    - **旋转的象**

      设 $ \cal R_\bm v^\psi $ 将 $ \bm P = x \bm i + y \bm j + z \bm k $ 旋转到 $ \wt{\bm P} $:

      在上式中令 $ \theta = \pi $, 则二元旋转 $ \R_\bm P^\theta $ 和 $ \R_{\wt{\bm P}}^\pi $ 正是纯四元数, 故有
      $$
  ...
  - 若 $ \bb P $ 与 $ \bb A $ 为纯四元数, 则
    ...
    - 若 $\bb A$ 为单位纯四元数, 即 $ \bb A^2 = -1 $,
      ...
      - 考虑变换 $ \bb P \mapsto \bb P' = \bb A \bb P \bb A $,
        - $ \bb P' $ 为纯四元数 (因为 $ \bb A $ 是一个旋转).
        - $ \vqty{\bb P'} = \vqty{\bb P} $, 于是该变换表示空间中的运动.
