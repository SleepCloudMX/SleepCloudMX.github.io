<p align="center">第 7 章  非线性方程与方程组的数值解法</p>
...
1. $ \forall x \in [a, b] \colon a \le \varphi(x) \le b $.
2. $ \exist L \in (0, 1), \forall x, y \in [a, b] \colon \vqty{\varphi(x) - \varphi(y)} \le L \vqty{x - y} $.

则 $ \varphi(x) $ 在 $ [a, b] $ 上存在唯一的不动点 $ x^* $.

**备注**	

- 若满足条件一，则存在不动点；若进一步满足条件二，则存在唯一不动点.
- 若 $ \forall x \in [a, b] \colon \vqty{\varphi'(x)} \le L < 1 $，则可推出条件二.

---

**定理 2**	设 $ \varphi(x) \in C[a, b] $ 满足上述两个条件，则对 $ \forall x_0 \in [a, b] $，不动点迭代法得到的迭代序列 $ \Bqty{x_k} $ 手来你到不动点 $ x^* $，并有
$$
\vqty{x_k - x^*} \le \dfrac{L^k}{1 - L} \vqty{x_1 - x_0}.
$$
**证明**
...
$$
\vqty{x_k - x^*} = \vqty{
	\varphi(x_{k-1}) - \varphi(x^*)
} \le L \vqty{x_{k-1} - x^*}
\le \cdots \le L^k \vqty{x_0 - x^*}.
$$
从而 $ \clim k x_k = x^* $.
...
\begin{align}
\vqty{x_{k+p} - x_k}
&\le \vqty{x_{k+p} - x_{k+p-1}} +
\vqty{x_{k+p-1} - x_{k+p-2}} + \cdots +
\vqty{x_{k+1} - x_k}
\\
&\le \pqty{
	L^{k+p-1} + L^{k+p-2} + \cdots + L^k
} \vqty{x_1 - x_0}
\le \dfrac{L^k}{1 - L} \vqty{x_1 - x_0},
\end{align}
$$
令 $ p \to \infty $，即得待证表达式. 证毕.

---

**推论**	类似可得 $ \vqty{x_k - x^*} \le \dfrac{1}{1 - L} \vqty{x_{k+1} - x_k} $，可作为终止条件.
