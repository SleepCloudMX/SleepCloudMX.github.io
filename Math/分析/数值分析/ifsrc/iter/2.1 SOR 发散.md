...
**定理 i-2-1（必要条件）**	若求解 $ \bm A \bm x = \bm b $ 的 SOR 迭代法收敛，则 $ 0 < \omega < 2 $.

**证明**
$$
\begin{align}
\det(\bm L_\omega) &= \det\bqty{
	(\bm D - \omega \bm L)\inv ((1 - \omega) \bm D + \omega \bm U)
} \\
&= \det\bqty{(\bm D - \omega \bm L)\inv}
