...
**定理 i-2-4（充分条件）**	若 $ 0 < \omega \le 1 $ 且 $ \bm A $ 严格对角占优或不可约弱对角占优，则求解  $ \bm A \bm x = \bm b $ 的 SOR 迭代法收敛.
...
\begin{cases}
	\wt{\bm x}^{(k+1)} = \wt{\bm B} \bm x^{(k)} + \wt{\bm f}, \\
	\bm x^* = \wt{\bm B} \bm x^* + \wt{\bm f}.
\end{cases} \QRA \begin{align}
	\Norm{\wt{\bm x}^{(k+1)} - \bm x^*}
	&\le \Norm{\wt{\bm B}} \Norm{\bm x^{(k)} - \bm x^*}
	= \wt q \Norm{\bm \ve^{(k)}}.
\end{align}
$$
该式本身不构成地推关系，但是由此有
...
&\le \vqty{1 - \omega} \cdot \Norm{\bm \ve^{(k)}} +
\vqty{\omega} \cdot \wt q \cdot \Norm{
	\bm x^{(k)} - \bm x^*
} \\
&= \pqty{
	\vqty{1 - \omega} + \vqty{\omega} \wt q
} \Norm{\bm \ve^{(k)}}
\\
&\le \pqty{
	\vqty{1 - \omega} + \vqty{\omega} \wt q
...
\end{align}
$$
其中 $ \vqty{1 - \omega} + \vqty{\omega} \wt q < 1 $，故 SOR 迭代法收敛.

**备注**	

- 实际上，若 GS 迭代矩阵的谱半径为 $ \wt q < 1 $，则只需 $ \omega \in \pbqty{0, \dfrac{2}{1 + \wt q}} $，即有 SOR 迭代法收敛.
- 注意到，当 $ \wt q \to 1 $ 时，$ \omega $ 的上界也相应变为 2，这与 $ \omega \in (0, 2) $ 的必要条件并不矛盾.
