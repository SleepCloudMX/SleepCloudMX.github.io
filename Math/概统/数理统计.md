...
利用 $ \overline{X} - \overline{Y} \sim N\pqty{\mu_1 - \mu_2, \dfrac{\sigma^2}{n_1} + \dfrac{\sigma^2}{n_2}} $, 记 $ \mu_0 = \mu_1 - \mu_2 $, $ U = \dfrac{
...
- $ \varphi:{} $当 $ \mathrm{SS} \ge \sigma_0^2 \chi_n^2(1 - \alpha) $ 时接受 $H_0$, 否则否定 $H_0$.
- $ \varphi':{} $当 $ \mathrm{SS} \le \sigma_0^2 \chi_n^2(\alpha) $ 时接受 $H_0'$, 否则否定 $H_0'$.
- $ \varphi'':{} $当 $ \sigma_0^2 \chi_n^2\pqty{1 - \dfrac{\alpha}{2}} \le \mathrm{SS} \le \sigma_0^2 \chi_n^2\pqty{\dfrac{\alpha}{2}} $ 时接受 $H''_0$, 否则否定 $H''_0$.
...
若认为 $ X \sim H_0: P(X = a_i) = p_i\ (i = \oneto{k}) $, 设对 $X$ 进行了足够多的 $n$ 次实验, $ \soneto{X}{n} $ 中等于 $a_i$ 的个数记作 $\nu_i$, 称为 $a_i$ 这个 "类" 的 **经验值** 或 **观察值**, 其近似于**理论值** $np_i$.

**定理 3.1** :star:	若原假设 $H_0$ 成立, 则 $ Z = \dsum_{i=1}^k \dfrac{(np_i - \nu_i)^2}{np_i} \dot\sim \chi_{k-1}^2 $.
...
**定理 3.2** :star:	在一定条件下, 若原假设 $H_0$ 成立, 使用极大似然估计出 $ (\soneto{\hat\theta}{r}) $, 并由此算出 $ p_i = p_i(\soneto{\hat \theta}{r}) $, 则 $ Z = \dsum_{i=1}^k \dfrac{(np_i - \nu_i)^2}{np_i} \dot\sim \chi_{k-1-r}^2 $.
