...
注: 以下偏度系数定义为 $ \beta_1 = \mu_3/\mu_2^{3/2} $，峰度系数定义为 $ \beta_2 = \mu_4/\mu_2^2 $.
...
- 常用统计量
  - 众数 $ M_0 = \dfrac{\alpha - 1}{\alpha + \beta - 2} $. (伯努利分布参数的矩估计)
...
- 概率密度函数 $ f(x; \alpha, \beta) = \dfrac{\beta^\alpha}{\Gamma(\alpha)} x^{\alpha - 1} \e^{-\beta x} \quad (x > 0) $.
  - $\alpha$ 称为形状参数.
  - $\beta$ 称为逆尺度参数.
- 累积分布函数 $ F(x; \alpha, \beta) = \dfrac{\gamma(\alpha, x)}{\Gamma(\alpha)} $.
...
- 函数运算
  ...
  - 可加性
    ...
    - 特例 2 (指数分布): 若 $X_i$ 独立同分布 $E(\lambda)$, 则 $ 2\lambda(\splus{X}{n}) \sim \chi_n^2 $.
