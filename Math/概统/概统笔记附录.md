...
- 特征函数 $ g(t) = E(\e^{\i t X}) = \dfrac{\e^{\tfrac{\i\mu t}{\lambda}}}{1 + \beta^2 t^2} $.

##### 7.3	其它性质

- $ a\mathrm{La}(\mu, \gamma) + b \sim \mathrm{La}(a\mu + b, a\lambda) $.

  $ \dfrac{\mathrm{La(\mu, \gamma)} - \mu}{\gamma} \sim \mathrm{La}(0, 1) $.
