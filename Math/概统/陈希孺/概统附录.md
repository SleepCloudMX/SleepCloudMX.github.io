...
陈希孺概统书上采用的是 $ P(X=i) = p(1-p)^{i} $ 的定义, 为了避免歧义, 我暂时将其记为 $GE_0(p)$. 这样定义是为了使得

1. $ GE_0(p) \sim NB(1, p) $.
2. 若 $\soneto{X}{n}$ 独立同分布 $GE_0(p)$, 则 $ \splus{X}{n} \sim NB(r, p) $.

这里一律使用更常见的上述定义, 即 $ GE(p) \sim GE_0(p) + 1 $. 如果要说的话, 这么做也是有好处的

几何分布和负二项分布似乎是定义分歧的重灾区, 建议在正式使用时预先说明所选取的定义.

##### 5.2	数字特征

- 数学期望：$1/p$.
- 母函数: $ G(s) = \dfrac{ps}{1-qs},\, s \in \pqty{-\dfrac{1}{q}, \dfrac{1}{q}} $.

##### 5.3	其它性质

- 几何分布具有无记忆性.
- 若 $\soneto{X}{r}$ 独立同分布 $GE(p)$, 则 $ \splus{X}{r} - r \sim NB(r, p) $.
...
- 相互独立的正态分布的运算
  ...
  - <font color=blue>分布之差</font>

    若 $ X_1 \sim N(\mu_1, \sigma_1^2),\, X_2 \sim N(\mu_2, \sigma_2^2) $ 相互独立, 则 $ X_1 - X_2 \sim N(\mu_1 - \mu_2, \sigma_1^2 + \sigma_2^2) $.

  - <font color=blue>分布之商</font>

    若 $X_1$ 和 $X_2$ 独立同分布 $N(0, 1)$, 则 $ X_1 / X_2 \sim C(1, 0) $ (柯西分布).

  - <font color=blue>分布之积</font>
  ...
  - <font color=blue>平方之和</font>

    若 $\soneto{X}{n}$ 独立同分布 $N(0, 1)$, 则 $Y = X_1^2 + X_2^2 +\cdots + X_n^2 \sim \chi_n^2$.

- 统计量的分布

  - $ \sqrt{n} (\overline{X} - \mu) / \sigma \sim N(0, 1) $. (标准差已知)
  - $\sqrt{n}\, (\overline{X} - \mu) / S \sim t_{n-1}$. (标准差未知)
