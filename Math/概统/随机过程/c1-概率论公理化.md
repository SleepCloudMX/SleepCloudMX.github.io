...
**备注**	$ \sigma $ 域不是域, $ \sigma $ 代数也不是代数 (一种代数结构). 因为域和代数都要求有一种运算构成阿贝尔群, 而交集与并集都不构成群.
...
= \int_a^b g(x) F(\dx).
$$
**备注**	当 $ F(x) = x $ 时, R-S 积分化为 Riemann 积分. 可类似定义反常积分. 这种推广有助于统一表达离散型与连续型的公式, 如[拉氏变换](# 1.3	矩母函数、特征函数和拉氏变换).
...
**备注**	如果 $X$ 是连续型随机变换, 则 $ F_X(x) $ 的 L-S 变换退化为对 p.d.f. $ f_X(x) $ 的 Laplace 变换.
...
**定义**	一随机过程 $X_T$, 若对 $ \forall t_1, \cdots, t_n \in T $, 与 $ h > 0 $ , 满足 $ (X_{t_1}, \cdots, X_{t_n}) $ 与 $ (X_{t_1 + h}, \cdots, X_{t_n + h}) $ 有相同的联合分布, 则称该过程未 **严平稳过程** (strictly stationary process).
