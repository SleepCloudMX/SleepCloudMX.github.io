...
X_{n + 1} = (X_n - 1)^+ + \xi_n,
$$
且 $ \Bqty{X_n, n \ge 0} $ 是一个马氏链. 设 $ P\Bqty{\xi_n = k} = a_k \ge 0 $, $ \dsum_{k = 0}^\infty a_k = 1 $, 则转义概率为
$$
\begin{cases}
...
\end{align}
$$
并称 $ \Bqty{\pi_i(0), i \in S} $ 为马氏链的初始分布.
...
**备注**	令 $ S(i) = \nosum I_\Bqty{X_n = i} $ 表示马氏链到达 $i$ 的次数, 于是 $ E[S(i) | X_0 = i] = \nosum p_{ii}^{(n)} $.
...
**备注**	若系统从某非常返状态触发, 则可能一直在非常返集 $T$ 中运动, 也可能进入某一基本常返闭集 $ C_h $ 中并一直在其中运动.
...
欲求 $ \nlim P\Bqty{X_n = i} \equiv \pi_i(n) $, 可研究 $ \nlim p_{ij}^{(n)} $.
...
- $ g(i, \lambda) = E\Bqty{\lambda^\tau \mid X_0 = i} $, $ \bm g(\lambda) = \pqty{g(i, \lambda), i \in S} $, $ g(\lambda) = E(\lambda^\tau) = \dsum_{k=0}^\infty g_k \lambda^k $.

#### 3.6.2  概率分布及其母函数

**引理**	若矩阵 $ \bm Q $ 满足 $ \nlim \bm Q^n = 0 $, 则 $ (\bm I - \bm Q)\inv $ 存在, 且
$$
...
1. $ g_k = \begin{cases}
   	\alpha_0, & k = 0, \\
   	\alpha \bm P^{k-1} \bm P_0, & k \ge 1.
   \end{cases} $ 
2. $ \bm g_k = \begin{cases}
   	\bm e, & k = 0, \\
   	\bm P^{k-1} \bm P_0, & k \ge 1.
   \end{cases} $ 
3. $ g(\lambda) = \alpha_0 + \lambda \alpha (\bm I - \lambda \bm P)\inv \bm P_0 $.
...
4. 记从 0 时刻和 1 时刻进入目标集之前的总报酬 (或性能指标) 为
   ...
   $$
   于是 $ W_0 $ 时定义于 $ \Bqty{X_n, n \ge 0} $ 上的可加泛函, 且 $ W_0 $ 和 $ W_1 $ 都是非负随机变量.
...
5. 原点矩、分布函数、矩母函数
   ...
   \end{align}
   $$
   并令 $ \bm r^{(k)}, \bm \mu^{(k)}, \bm \phi(\lambda) $ 分别为 $ r_i^{(k)}, \mu_i^{(k)}, \phi_i(\lambda)\ (i \in \overline H) $ 的列向量.
