...
&= \lim_{t \to \infty} \dfrac{
	\sum_{n = 1}^{N(t)} R_n
}{N(t)} \cdot \dfrac{N(t)}{t}
\\
&= E(R) \cdot \dfrac{1}{E(X)}.
\end{align}
$$
...
$$
\begin{align}
g(t) &= \intoi E(R_{N(t) + 1} \mid X_1 = x) \dd{F(x)}
\\
&= h(t) + \int_0^t g(t - x) \dd{F(x)}.
\end{align}
$$
...
$$
\begin{align}
\vqty{h(t)} &\le \int_t^{+\infty} \vqty{
	E(R_1 \mid X_1 = x)
} \dd{F(x)}
\\
&\le \int_t^{+\infty} E\pqty{
	\vqty{R_1} \mid X_1 = x
} \dd{F(x)}
\\
&\le \intoi E(R_1 \mid X_1 = x) \dd{F(x)}
\\
&= E(R_1) < \infty.
\end{align}
$$
